document.addEventListener("DOMContentLoaded", function() {
  var bars = document.querySelectorAll('.bar');
  //var circles = document.querySelectorAll('.cbar');
  bars.forEach(function(bar) {
    var percent = bar.closest('li').getAttribute('data-percent');

    bar.style.width = percent + '%';
    bar.style.backgroundColor = 'yellow'; // Set background color to yellow
  });

    // Constants for SVG circle circumference
    //var circles = document.querySelectorAll('li');
    var cbar = document.querySelectorAll('.cbar');
    const circleCircumference = 2 * Math.PI * 45; // 2 * PI * radius (45)

    // Apply percentage fill dynamically
    cbar.forEach(function(cbar) {
      var percent = cbar.closest('li').getAttribute('data-percent');

      // Calculate stroke-dasharray based on percentage
      const dashArray = (percent / 100) * circleCircumference;
      cbar.style.strokeDasharray = `${dashArray} ${circleCircumference}`;

    }
  );
});



(function() {
    $(".skills-prog li")
      .find(".skills-bar")
      .each(function(i) {
        $(this)
          .find(".bar")
          .delay(i * 150)
          .animate(
            {
              width:
                $(this)
                  .parents()
                  .attr("data-percent") + "%"
            },
            1000,
            "linear",
            function() {
              return $(this).css({
                "transition-duration": ".5s"
              });
            }
          );
      });

  }.call(this));
  
   //#sourceMappingURL=data:application/json;base64,
  //# sourceURL=coffeescript
  
